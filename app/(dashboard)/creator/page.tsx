...
export default function CreatorPage() {
  return (
    <div className="mx-auto max-w-5xl">
      <Link
        href="/dashboard"
        className="mb-8 inline-flex items-center gap-1 text-xs font-body text-ns-muted transition-colors hover:text-ns-secondary"
      >
      ...
      <header className="mb-8 border-b border-ns-border pb-8">
        <div className="flex items-start gap-4">
          <span className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl border border-ns-secondary/25 bg-ns-secondary/10 text-ns-secondary">
            <UploadMovieIcon size={23} />
          </span>
          <div>
            <p className="text-[10px] font-body uppercase tracking-[0.22em] text-ns-secondary">Creator Studio</p>
        ...
        <ul className="mt-4 grid gap-3 md:grid-cols-3">
          {uploadDetails.map(detail => (
            <li key={detail} className="flex items-start gap-2 text-xs leading-relaxed font-body text-ns-muted">
              <CheckIcon size={14} className="mt-0.5 flex-shrink-0 text-ns-secondary" />
              {detail}
            </li>
          ))}
        </ul>
