  ...
  if (feed.length === 0) {
    return (
      <div className="py-16 text-center">
        <FriendsIcon size={44} className="text-ns-secondary/40 mx-auto mb-4" />
        ...
        <Link
          href="/friends/find"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-ns-secondary text-ns-bg text-sm font-body font-medium hover:bg-amber-400 transition-colors"
        >
        ...
        <div key={event.id} className="flex items-start gap-3 p-4 bg-ns-surface border border-ns-border rounded-2xl hover:border-ns-border/80 transition-colors">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-body text-white leading-snug">
              <Link href={`/profile/${event.authorUsername}`} className="text-ns-secondary hover:text-amber-400 transition-colors font-medium">
            ...
            <div className="flex items-center gap-2 mt-1">
              {(() => { const Ico = EVENT_ICONS[event.type] ?? FilmIcon; return <Ico size={12} className="text-ns-secondary/60 flex-shrink-0" /> })()}
          ...
          {event.type === 'rated_movie' && typeof event.data.tmdbId === 'number' && (
            <Link
              ...
              className="text-[10px] font-body text-ns-muted hover:text-ns-secondary transition-colors flex-shrink-0 flex items-center gap-0.5"
            >
